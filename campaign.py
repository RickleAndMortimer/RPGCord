...
async def beginnerCampaign(ctx):
    ...
    if max == 0:
        await ctx.channel.send("Your party fought the goblins. The battle wasn't very hard, but it was a valiant effort from the goblin's side as they did -5 damage to your whole party")
    elif max == 1:
...
def getCampaign_running():
	return campaign_running
	
async def join(ctx):
	if getCampaign_running:
		#add user into an adventurer
